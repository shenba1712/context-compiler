...
The demo’s “Run agent” button is the controlled version of that story. You ask a question and set the same token budget as Compile — that budget is the agent’s reading ceiling. The model compiles under it, reads the omitted-sections manifest, and decides what to do next — expand a specific section (while staying under the ceiling), or answer. Each step streams live. A token meter climbs as it reads, next to the crossed-out whole-file count. If the whole file already fits your budget, it skips retrieval and answers once. The loop is bounded on purpose: a max-steps cap, your token ceiling, and a fail-safe that turns any unusable decision (bad JSON, unknown section, a recompile that would not grow) into “answer with what we have” rather than a spin. It needs an LLM key and rides the same Gemini → OpenRouter failover chain as everything else.
...
Budget by question breadth, not file size. A factual lookup is happy around a thousand tokens. Synthesis across a few sections wants something like four thousand. “Summarize everything” needs a budget at least as large as the file — at which point the compiler returns the whole document as lossless passthrough. The same control is the Agent’s reading ceiling on the demo.
