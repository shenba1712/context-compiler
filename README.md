...
For Compile, the budget is a hard packing ceiling: output stays under the slider. UI expands and Agent expands are different — they can raise the effective context above that number (with a warning on Prove, and a soft stop on Agent). When ranking shows a clear relevance drop-off, packing stops early and returns less than you allowed (`CC_RELEVANCE_FLOOR`, default 0.4 × the top score). On vague questions with no clear signal, the packer fills the budget as recall insurance. Leaving the slider at four thousand is almost always fine.
